...
public class CollectionRepository : ICollectionRepository
{
    ...
    public async Task DeleteAsync(int id, int userId)
    {
        ...
        // also remove book links
        var members = _db.BookCollections.Where(bc => bc.CollectionId == id);
        _db.BookCollections.RemoveRange(members);
        var rules = _db.CollectionRules.Where(r => r.CollectionId == id);
        _db.CollectionRules.RemoveRange(rules);
        await _db.SaveChangesAsync();
    }
    ...
    {
        return await _db.BookCollections.CountAsync(bc => bc.CollectionId == collectionId);
    }

    public async Task<CollectionRuleEntity> AddRuleAsync(CollectionRuleEntity rule)
    {
        _db.CollectionRules.Add(rule);
        await _db.SaveChangesAsync();
        return rule;
    }
    ...
    {
        var r = await _db.CollectionRules.FindAsync(id);
        if (r == null) return;
        _db.CollectionRules.Remove(r);
        await _db.SaveChangesAsync();
    }

    public async Task<CollectionRuleEntity[]> GetRulesForCollectionAsync(int collectionId)
    {
        return await _db.CollectionRules.Where(r => r.CollectionId == collectionId).ToArrayAsync();
    }
}
